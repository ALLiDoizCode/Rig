 ...
 *
 * Acceptance Criteria covered: #1, #3, #7, #8, #9
 */
...
describe('useRepositories', () => {
  ...
  it('[P0] should set status to error when fetchRepositories throws RigError', async () => {
  ...
  it('[P1] should return stale data immediately while refetch is in-flight', async () => {
    ...
    vi.mocked(fetchRepositories).mockResolvedValue(staleRepos)
    ...
    let resolveRefetch: (value: typeof freshRepos) => void
    const refetchPromise = new Promise<typeof freshRepos>((resolve) => {
      resolveRefetch = resolve
    })
    vi.mocked(fetchRepositories).mockReturnValue(refetchPromise)
  ...
  it('[P0] should display repositories even when partial relay data is returned', async () => {
    ...
    vi.mocked(fetchRepositories).mockResolvedValue(partialRepos)
  ...
  it('[P1] should use arrow wrapper for queryFn (not bare function reference)', async () => {
    // Given: fetchRepositories is mocked
    vi.mocked(fetchRepositories).mockResolvedValue([])
    ...
    await waitFor(() => {
      // Then: fetchRepositories should be called without arguments
      // (arrow wrapper prevents QueryFunctionContext from being passed)
  ...
  it('[P2] should configure staleTime of 1 hour for repository data', async () => {
    ...
    vi.mocked(fetchRepositories).mockResolvedValue(createRepositories(1))
    ...
    // (We verify by checking fetchRepositories is not called again within staleTime)
  ...
  it('[P1] should update displayed data when refetch returns fresh data', async () => {
    ...
    vi.mocked(fetchRepositories).mockResolvedValue(initialRepos)
    ...
    vi.mocked(fetchRepositories).mockResolvedValue(updatedRepos)
